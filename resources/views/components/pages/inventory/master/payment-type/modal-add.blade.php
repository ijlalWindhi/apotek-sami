{{-- Button Add --}}
<x-button color="blue" data-modal-target="modal-add-payment-type" data-modal-toggle="modal-add-payment-type">
    <i class="fa-solid fa-plus"></i>
    <span class="ms-2">Tambah</span>
</x-button>

{{-- Modal --}}
<div id="modal-add-payment-type" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                    Tambah Master Tipe Pembayaran
                </h3>
                <button type="button"
                    class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                    data-modal-toggle="modal-add-payment-type">
                    <i class="fa-solid fa-xmark"></i>
                    <span class="sr-only">Close modal</span>
                </button>
            </div>
            <!-- Modal body -->
            <form class="p-4 md:p-5 max-h-[calc(100vh-8rem)] overflow-y-auto" id="create" method="POST">
                @csrf
...
<script>
    ...
    $('#modal-add-payment-type form').on('submit', function(e) {
        e.preventDefault();
        ...
        // Show loading icon
        $('#modal-add-payment-type form').prepend(templates.loadingModal);
        $.ajax({
            url: `/inventory/master/payment-type`,
            type: "POST",
            ...
            success: function(response) {
                // Close modal
                $('#modal-add-payment-type').removeClass('flex').addClass('hidden');
                Swal.fire({
                    icon: "success",
            ...
            complete: function() {
                // Hide loading icon
                $('#modal-add-payment-type form .absolute').remove();
            }
        });
    });
</script>
