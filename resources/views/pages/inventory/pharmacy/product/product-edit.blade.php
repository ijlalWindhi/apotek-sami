        ...
        fetchData: () => {
            ...
            $.ajax({
                ...
                success: async (response) => {
                    ...
                    // Handle margin if enabled
                    if (data.show_margin) {
                        const marginToggle = document.querySelector(SELECTORS.marginToggle);
                        marginToggle.checked = true;
                        UIManager.toggleMarginFields(true);

                        // Calculate and display margin
                        const purchasePrice = UIManager.parseCurrency(data.purchase_price);
                        const sellingPrice = UIManager.parseCurrency(data.selling_price);
                        const margin = PriceCalculator.calculateMargin(purchasePrice, sellingPrice);
                        form.find('input#margin_display').val(UIManager.formatPercentage(margin *
                            100));
                        form.find('input#margin_percentage').val(data.margin_percentage);
                    }

                    // Handle unit conversions
                    if (data.unit_conversions && data.unit_conversions.length > 0) {
                        data.unit_conversions.forEach((conversion, index) => {
                            const label = index === 0 ?
                            ...
                            const unitSelect = `
                                                    ...
                                                    <div class="flex w-full justify-between items-center gap-2 additional-unit">
                                                        <div class="flex w-fit items-center gap-2 bg-gray-50 border border-gray-300 rounded-lg">
                                                            <input type="number" name="additional_conversion_${unitCount}" 
                                                                value="${parseFloat(conversion.from_value)}"
                                                                class="bg-gray-50 border-transparent text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-20 p-2.5" required />
                                                            X
                                                        ...
                                                        <div class="flex w-fit items-center bg-gray-50 border border-gray-300 rounded-lg">
                                                            <input type="number" name="additional_conversion_result_${unitCount}" 
                                                                value="${parseFloat(conversion.to_value)}"
                                                                class="bg-gray-50 border-transparent text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-20 p-2.5" required />
                                                            <span class="text-gray-900 text-sm p-2.5">${conversion.from_unit.name}</span>
                                                        </div>
                                                        <button type="button"
                                                            class="btn-remove-unit text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-4 py-2.5 text-center">
        ...
        init: () => {
            ...
            $('#btn-add-unit').on('click', function(e) {
                ...
                <div class="flex w-full justify-between items-center gap-2 additional-unit">
                    ...
                    =
                    <div class="flex w-fit items-center bg-gray-50 border border-gray-300 rounded-lg">
                        <input type="number" name="additional_conversion_result_${unitCount}" value="1"
                            class="bg-gray-50 border-transparent text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-20 p-2.5" required />
                        <span class="text-gray-900 text-sm p-2.5">${selectedUnitText}</span>
                    </div>
                    <button type="button"
                        class="btn-remove-unit text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-4 py-2.5 text-center">
                        <i class="fa-solid fa-trash"></i>
            ...
            $('body').on('click', '.btn-remove-unit', function() {
                ...
                const remainingUnits = $('.additional-unit').length;

                if (remainingUnits === 1) { // Only the label remains
                    $('.additional-unit').remove();
                    unitCount = 0;
                } else {
                    unitCount = remainingUnits - 1; // Subtract 1 for the label
                }
            });
